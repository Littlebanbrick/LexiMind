...
Distribution variant: this is the API server AND the static file server (no
nginx). The LexiMind_development variant is API-only (behind nginx); keep the
shared query/rate-limit/daily-reading logic in sync between the two, but do
not duplicate static serving in the dev variant.

NOTE: this file is EXCLUDED from sync.py, so it must be edited by hand here
whenever shared backend logic changes.
"""
